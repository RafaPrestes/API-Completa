...
public class PerfilController {
    ...
    public Perfil getById(@PathVariable int id){
        return perfilRepository.findById(id).orElseThrow(() -> new ResponseStatusException(
            HttpStatus.NOT_FOUND, "Perfil não encontrado."));      
    }

    @PostMapping()
    @ResponseStatus(HttpStatus.CREATED) //atualizando o status padrão que é OK para CREATED
    ...
    public void delete(@PathVariable int id){
        perfilRepository.findById(id).map(perfil -> {
    ...
    public void update(@PathVariable int id,
     @RequestBody Perfil perfil){
            perfilRepository.findById(id).map(perfilExistente ->{
